...
for directory in directories:
    ...
    for fn in a:
        ...
        for col in range(ncols):
            ...
            plt.subplot(332)
            plt.title('htrm')
            plt.semilogy(htrmcols[:,col],pzmcols[:,col],'-o',ls=ls,c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)
            plt.axvline(-0.03,ls='--')
            plt.axvline(0.03,ls='--')
            ...
            plt.subplot(333)
            plt.title('totuflum')
            plt.semilogy(totuflumcols[:,col],pzmcols[:,col],'-o',ls=ls,c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)
            
            plt.subplot(334)
            plt.title('totdfllum')
            plt.semilogy(totdflumcols[:,col],pzmcols[:,col],'-o',ls=ls,c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)

            plt.subplot(335)
            plt.title('abs_o3')
            plt.semilogy(A_oz_lcols[:,col]*1362./4.,pzmcols[1:,col],'-o',ls=ls,c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)      

            plt.subplot(336)
            plt.title('abspn')
            plt.semilogy(abspncols[:,col]*1362./4.,pzmcols[1:,col],'-o',ls=ls,c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)

            plt.subplot(337)
            plt.title('tavelm')
            plt.semilogy(tavelmcols[:,col],pzmcols[1:,col],'-o',label=str(fn),c=colors[i3])
            plt.ylim(pzmcols[0,col]*1.1,1)
            # plt.legend()
