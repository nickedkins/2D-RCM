...
C        level and the heating rate for each layer
     ...
     *                RADCN1,RADCN2 
     ...
     &                   TAUTBL(0:NTBL),TRANS(0:NTBL), TF(0:NTBL)
      ...
      COMMON /CVRSN1A/   HNAMKG1A,HVRKG1A
     ...
     *             HNAMRDI
      ...
      CHARACTER*18 HVRKG1A,HVRKG1,HVRKG2,HVRKG3,HVRKG4,HVRKG5,
     *             HVRKG6,HVRKG7,HVRKG8,HVRKG9,HVRKG10,
     *             HVRKG11,
     *             HVRKG12,HVRKG13,HVRKG14,HVRKG15,HVRKG16

      CHARACTER*18 HNAMKG1A,HNAMKG1,HNAMKG2,HNAMKG3,HNAMKG4,HNAMKG5,
...
C
 ...
 3000    CONTINUE
         ...
         IF (IOUT .LE. 40 .OR. IFLAG .EQ. 15) GO TO 3500
         IF (IFLAG .EQ. 99) THEN
            IFLAG = 1
...
C     
     ...
     *     HNAMKG10,HVRKG10,HNAMKG12,HVRKG12,
...
!         CLOSE(IWR)
 ...
 9952 FORMAT(1X,I3,9X,F7.6,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9953 FORMAT(1X,I3,9X,F7.5,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9954 FORMAT(1X,I3,9X,F7.4,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9955 FORMAT(1X,I3,9X,F7.3,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9956 FORMAT(1X,I3,9X,F7.2,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9957 FORMAT(1X,I3,9X,F7.1,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 9958 FORMAT(1X,I3,9X,F7.1,3X,F8.4,6X,F8.4,6X,F12.7,8x,F11.5)
 ...
 9910 FORMAT('  Modules and versions used in this calculation:',/,/,
     *         16(5X,a18,2X,A18,10X, a18,2X,A18,/))
!      STOP
      return !NJE
      END
...
!      READ (IRD,9012) TBOUND,IEMISS,IREFLECT,(SEMIS(I),I=1,16)
      tbound=tboundm; iemiss=iemissm; 
     & ireflect=ireflecm; semis=semism !NJE
...
! 2000    CONTINUE                                                            
           	...
           	pavel=pavelm; tavel=tavelm; 
     &  altz=altzm; 
     &tz=tzm;pz=pzm;wkl=wklm;
     &wbrodl=wbrodlm !NJE
...
C        Left-justify all inputed names.                                      
   ...
   40 CONTINUE                                                            
     ...
     *     HNAMLPK / '         LINPAK.f:' /
      ...
      DATA WAVENUM1(1) /10./, WAVENUM2(1) /120./, DELWAVE(1) /110./
      DATA WAVENUM1(2) /120./, WAVENUM2(2) /350./, DELWAVE(2) /230./
      DATA WAVENUM1(3) /350./, WAVENUM2(3) /500./, DELWAVE(3) /150./
      DATA WAVENUM1(4) /500./, WAVENUM2(4) /630./, DELWAVE(4) /130./
      DATA WAVENUM1(5) /630./, WAVENUM2(5) /700./, DELWAVE(5) /70./
      DATA WAVENUM1(6) /700./, WAVENUM2(6) /820./, DELWAVE(6) /120./
      DATA WAVENUM1(7) /820./, WAVENUM2(7) /1000./, DELWAVE(7) /180./
      DATA WAVENUM1(8) /1000/, WAVENUM2(8) /1110./, DELWAVE(8) /110./
      DATA WAVENUM1(9) /1110./, WAVENUM2(9) /1210./, DELWAVE(9) /100./
      DATA WAVENUM1(10) /1210./,WAVENUM2(10) /1430./,DELWAVE(10) /220./
      DATA WAVENUM1(11) /1430./,WAVENUM2(11) /1810./,DELWAVE(11) /380./
      DATA WAVENUM1(12) /1810./,WAVENUM2(12) /2080./,DELWAVE(12) /270./
      DATA WAVENUM1(13) /2080./,WAVENUM2(13) /2250./,DELWAVE(13) /170./
      DATA WAVENUM1(14) /2250./,WAVENUM2(14) /2700./,DELWAVE(14) /450./
      DATA WAVENUM1(15) /2700./,WAVENUM2(15) /3250./,DELWAVE(15) /550./

      DATA NG /16,16,16,16,16,16,16,16,16,16,16,16,16,16,16,16/
      DATA NSPA /9,9,9,9,9,9,9,9,9,9,9,9,9,1,9,9/
      DATA NSPB /1,1,5,5,5,0,1,1,1,1,1,0,0,1,0,1/
