...
C          OPEN (IWR,FILE='OUTPUT_RRTM',FORM='FORMATTED')
C          WRITE(IWR,9899)WAVENUM1(ISTART),WAVENUM2(IEND),IATMOS
C          WRITE(IWR,9900)
C          WRITE(IWR,9901)
C C
C          DO 3000 I = NLAYERS, 0, -1
C             IF (PZ(I) .LT. 1.E-2) THEN
C                WRITE(IWR,9952) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSEIF (PZ(I) .LT. 1.E-1) THEN
C                WRITE(IWR,9953) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSEIF (PZ(I) .LT. 1.) THEN
C                WRITE(IWR,9954) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSEIF (PZ(I) .LT. 10.) THEN
C                WRITE(IWR,9955) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSEIF (PZ(I) .LT. 100.) THEN
C                WRITE(IWR,9956) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSEIF (PZ(I) .LT. 1000.) THEN
C                WRITE(IWR,9957) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ELSE
C                WRITE(IWR,9958) I,PZ(I),TOTUFLUX(I),TOTDFLUX(I),
C      &              FNET(I), HTR(I)
C             ENDIF
C  3000    CONTINUE
C          WRITE(IWR,9903)PAGE
...
C     
C          WRITE(IWR,9910) HNAMRTM,HVRRTM,HNAMATM,HVRATM,HNAMRTR,
C      *     HVRRTR,HNAMRTC,HVRRTC,HNAMREG,HVRREG,HNAMRGC,HVRRGC,
C      *     HNAMRTX,HVRRTX,HNAMRGX,HVRRGX,HNAMSET,HVRSET,
C      *     HNAMCLD,HVRCLD,HNAMUTL,HVRUTL,HNAMTAU,HVRTAU,
C      *     HNAMRDS,HVRRDS,HNAMDIS,HVRDIS,HNAMEXT,HVREXT,
C      *     HNAMERR,HVRERR,HNAMLPK,HVRLPK,HNAMRDI,HVRRDI,
C      *     HNAMKG1,HVRKG1,HNAMKG2,HVRKG2,HNAMKG3,HVRKG3,
C      *     HNAMKG4,HVRKG4,HNAMKG5,HVRKG5,HNAMKG6,HVRKG6,
C      *     HNAMKG7,HVRKG7,HNAMKG8,HVRKG8,HNAMKG9,HVRKG9,
C      *     HNAMKG10,HVRKG10,HNAMKG11,HVRKG11,HNAMKG12,HVRKG12,
C      *     HNAMKG13,HVRKG13,HNAMKG14,HVRKG14,HNAMKG15,HVRKG15,
C      *     HNAMKG16,HVRKG16
...
C       READ (IRD,9012) TBOUND
      tbound=tboundm; iemiss=iemissm; 
     & ireflect=ireflecm; semis=semism !NJE,IEMISS,IREFLECT,(SEMIS(I),I=1,16)
