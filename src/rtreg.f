...
c              if (iang.eq.1) write (*,'2I4,3e12.4')ig,lev,taug(lev,ig),
c    &             blay,plfrac
               ...
               ELSE
                  ...
                  if(lev.eq.1.AND.iang.eq.1) then
                     !print *,'DOWN ',ig,odepth,trans(itr)
                     !print *,tausfac, PLANKLEV(LEV-1,IBAND),blay,bbd
                     !print *, 'UP ',PLANKLEV(LEV,IBAND),bbu(lev,iang)
                   endif
 ...
 3000    CONTINUE 
         ...
         !print *,'fracs(1,ig) ',ig,iband,fracs(1,ig)
         !print *,plankbnd(iband), plankbnd(1)
         RAD0 = FRACS(1,IG) * PLANKBND(IBAND)
         !print *,'rad0 ',rad0,' numang ',numang
...
c        write(6,'(i5,e14.5,f10.4,e14.5)')  lev,FNET(LEV)-FNET(LAY),
c    &      PZ(LEV)-PZ(LAY),htr(lev)
 8000 CONTINUE

      !print *,'angweigh ',angweigh
      !print *,'fracs(1) ',sum(fracs(1,:))
      !print *,'plankbnd(1) ',plankbnd(1)
      !print *, delwave(1)*fluxfac*plankbnd(1)*0.5
      !print *, totuflux(0)
      RETURN
      END   
